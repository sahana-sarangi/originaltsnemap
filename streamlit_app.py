...
st.set_page_config(layout="wide", page_title="Oncology Visualization")
...
keep_cols = ['TSNE-x', 'TSNE-y', 'Cluster_Num', 'Cluster_ID_Full', 'Year', 'TopicName', 'AbstractTitle', 'Abstract', 'session_title', 'session_type']
...
final_chart = (
    ...
    .encode(
        ...
        color=alt.Color('Cluster_Num:N', title='Cluster ID', scale=alt.Scale(scheme='tableau20'),
            legend=alt.Legend(orient='right', columns=2, symbolLimit=100)),
        opacity=alt.condition(year_selection & session_selection & legend_selection, alt.value(0.9), alt.value(0.01)),
        tooltip=[
...
<head>
  ...
  <style>
    body {{ background-color: #f4f7f9; font-family: sans-serif; margin: 0; display: flex; flex-direction: column; align-items: center; }}
    .header-bar {{ width: 100%; background-color: #2c3e50; color: white; padding: 40px 0; text-align: center; margin-bottom: 50px; }}
    .card {{ background: white; border-radius: 16px; box-shadow: 0 20px 50px rgba(0,0,0,0.1); padding: 60px; width: fit-content; min-width: 1100px; display: flex; flex-direction: column; align-items: center; }}
    .vega-embed {{ display: flex !important; flex-direction: column; align-items: center; }}
    canvas.vega-canvas {{ order: 1; }}
    .vega-binds {{ order: 2; margin-top: 50px; display: flex; justify-content: center; gap: 40px; width: 100%; }}
    .vega-bind-name {{ font-weight: 600; margin-right: 12px; color: #2c3e50; }}
    select, input {{ padding: 10px 15px; border-radius: 8px; border: 1.5px solid #dcdde1; }}
  </style>
</head>
<body>
  <div class="header-bar">
    <h1>Oncology Research Visualization</h1>
    <p>Interactive t-SNE Clustering of Medical Abstracts</p>
  </div>
