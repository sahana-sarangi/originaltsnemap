...
<head>
  ...
  <style>
    ...
    .header-bar h1 {{ margin: 0; font-size: 28px; font-weight: 600; }}
    .header-bar p {{ margin: 8px 0 0 0; opacity: 0.8; font-size: 16px; }}
    .card {{
      background: white;
      border-radius: 12px;
    ...
    /* This centers the entire block (Chart + Legend) */
    #vis {{ 
      display: flex;
      flex-direction: column; /* Stack chart and binds vertically */
      align-items: center;
    }}
    /* Vega-embed creates a wrapper around the canvas and the binds */
    .vega-embed {{
      display: flex !important;
      flex-direction: column; /* Stack children vertically */
      align-items: center;
    }}
    /* Force the chart canvas to be on top */
    .vega-embed summary {{ order: 3; }} /* This moves the '...' menu if present */
    canvas.vega-canvas {{ order: 1; }}
    
    /* Force the binds (filters) to be at the bottom */
    .vega-binds {{
      order: 2; 
      margin-top: 30px; /* Space between chart and filters */
      display: flex;
